...
def calc_chevy(T,P,alpha):
    ...
    T_reduced_list = reduced_T(T,200,3000)
    P_reduced_list = calc_reduced_P(P,.01,10.0)
...
def plot_sens(dictonary,path,key_words):
    for key in dictonary.keys():
        for j, sens in enumerate(dictonary[key]):
            plt.figure()
            values = calc_chevy(np.arange(200,3000),[1]*(3000-200),sens)
            temp_list = np.arange(200,3000)
    ...
    for key in dictonary.keys():
        for j, sens in enumerate(dictonary[key]):
            ...
            for pressure in pressure_list:          
                values = calc_chevy(np.arange(200,3000),[pressure]*(3000-200),sens)
                temp_list = np.arange(200,3000)
...
lines=load_text_file('/home/jmp/MSI_Theory/PAPR-MESS_calculation/CH3+OHtripletabs/calculation_29/'+file_name)
...
build_reaction_string(p_list, temp_dict = {'R':'CH3 + OH', 'P8':'CH2 + H2O'})
...
plot_sens2(test,'/home/jmp/MSI_Theory/ch3+oh/Sensitivity Plots',key_words,[1])
