...
Temperature_list = np.arange(300,700,10).tolist() + np.arange(700,2500,100).tolist() # K
Pressure_list = [[1.], ['[atm]']] # Torr will be converted into atm internally

channels = ['P2->P3'] # channel-specific rate constants of interest

pertubation_percent = 0.1 # percentage of perturbation
nominal_MESS_input_path = os.getcwd() + '/h2no+oh/'
nominal_MESS_input = 'h2nooh.inp'
...
model.fit_Cheb_rates(n_P, n_T, P_min=0.0001, P_max=10.0, T_min=300.0, T_max=2500.0, same_line_result=same_line_result) # fit rate constants into Chebyshev expressions
